...
HEIC/HEIF → JPEG client-side · bake EXIF orientation in during compression · dedupe via `content_hash` unique per event · tab closed mid-upload → queue persists, resumes · cap in-flight queue size, free blobs after success · webhook/callback ordering → idempotent activation · large ZIPs → stream / chunk · anonymous guest allowed (attribute by token).
